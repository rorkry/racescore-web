...
// 競馬場情報
const RACECOURSE_INFO: Record<string, {
  direction: '右回り' | '左回り';
  straightLength: Record<string, number>;
  hasSlope: boolean;
  slopeDesc?: string;
  notes: string[];
}> = {
...
// コース特性生成関数
function generateCourseData(courseId: string, pattern: string, distanceToFirstCorner: number) {
  const parts = courseId.split('_');
  const racecourse = parts[0];
  const surface = parts[1] as '芝' | 'ダート';
  const distance = parseInt(parts[2], 10);
  const trackSize = parts[3] as '内' | '外' | undefined;

  const info = RACECOURSE_INFO[racecourse];
  if (!info) return null;

  // 直線長さを判定
  let straightLength: '短い' | '標準' | '長い';
  ...
  // 脚質適性
  let runningStyleAdvantage: string[];

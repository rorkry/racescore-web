...
### ソートロジックの修正（2024-12-20）
- [x] データなしの馬をレースカードの一番下に配置
- [x] 動作確認（スペルビア、ポムファッジが一番下に配置されることを確認）

### 馬名表示の修正（2024-12-20）
- [x] 馬名表示時に$マーク・*マークを除去


## 過去レース表示機能（2024-12-20）

### データベース変更
- [x] wakujunテーブルを日付ごとに保持するように変更
- [x] CSVアップロードAPIを修正（上書きではなく追加）

### 過去レースカード表示
- [x] 日付パラメータで過去のレースカードを表示
- [x] 利用可能な日付一覧をドロップダウンで表示

### 過去走詳細の拡張
- [x] 日付クリックでその開催のレースカードに遷移（データがある場合）
- [ ] 過去走詳細に「その時の競うスコア」を表示（※今後の実装案）
...
**概要:** 前半2Fや通過順位から、レースの隊列（誰が逃げて、誰が控えるか）を可視化
...
**テン指数計算式:**
- 近3走の平均テン2Fタイムを基本スコア
- 近3走の初角通過順位が「1番手」ならボーナス加算（逃げ癖補正）
- 今回の枠順補正（内枠の先行馬はプラス、外枠の先行馬は少し割引）
- 前走距離が今回距離よりも短かった場合、ボーナス加算（距離延長補正）

**UIデザイン:**
- 横長の「展開レーン」（左=ゴール、右=スタート）
- 馬番の丸アイコンが計算された位置に配置
- アイコンデザイン:
  - 枠番色の縁取りリング
  - 競うスコア50点以上: 金色の光彩
  - 競うスコア30点以上: 銀色の光彩
  - 脚質に応じた背景色（逃げ=赤、先行=オレンジ、中団=青、後方=灰色）
- ボタン切り替え時にアイコンがスムーズにスライド移動（CSS transition）

**必要データ:**
- [ ] 前半2Fタイム
- [ ] 上がり3Fタイム
- [x] 通過順位（corner_2, corner_3, corner_4）
- [x] 枠順（waku）
- [x] 距離（distance）

### 2. シナリオシミュレーター：「展開・馬場バイアスボタン」

**概要:** 「もしペースが遅くなったら？」「もし内が有利な馬場なら？」という仮説をボタン一つで反映

**ペース判定ボタン:**
- [S (スロー)]: 逃げ・先行馬の評価値を +10pt
- [M (平均)]: 補正なし
- [H (ハイ)]: 差し・追込馬かつ上がり3Fが速い馬に +10pt

**馬場バイアスボタン:**
- [内有利]: 1〜4枠の馬に +5pt
- [フラット]: 補正なし
- [外有利]: 5〜8枠の馬に +5pt
- [前残り]: 想定4コーナー通過順位が良い馬に +5pt

**UI:**
- 展開ボードの下にトグルボタンを設置
- ユーザーが押すと馬柱の「注目馬」の背景色がリアルタイムで変化
- おすすめ順にソート
...
## 指数CSVアップロードツール（2024-12-20）

### 概要
複数の指数CSVファイルを結合し、ワンクリックでDBにアップロードするツール

### 対象指数（6種類）
- 後半4F指数: `C:\競馬データ\L4F\2025`
- 前半2F指数: `C:\競馬データ\T2F\2025`
- ポテンシャル指数: `C:\競馬データ\ポテンシャル指数\2025`
- レボウマ: `C:\競馬データ\レボウマ\2025`
- 巻き返し指数: `C:\競馬データ\巻き返し指数\2025`
- クッション値: `C:\競馬データ\クッション値\2025`

### 実装項目
- [x] APIエンドポイント `/api/upload-indices` を作成
- [x] 指数データ保存用のDBテーブル `indices` を作成
- [x] TypeScriptスクリプト `tools/upload-indices.ts` を作成
- [x] Windowsバッチファイル `sync-indices.bat` を作成
- [x] 動作テスト（サンプルデータで成功確認）


## 指数データ表示テスト（2024-12-20）

### 概要
取り込んだ指数データをレースカードに表示するテスト実装

### 実装項目
- [ ] 指数データの構造とrace_idの形式を確認
- [ ] APIエンドポイントに指数データ取得機能を追加
- [ ] フロントエンドに指数表示を追加（馬の横に日付と各指数）
- [ ] 動作テスト
...
### 変更内容
- [x] テーブルの指数カラム（L4F, T2F, ポテ, レボ, 巻返, クッ）を削除

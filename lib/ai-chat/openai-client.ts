...
【最重要ルール】
${hasRaceData ? `
- 下記「参考情報」に各馬の過去走データと指数が含まれています
- このデータを**必ず分析して**、具体的な馬名と数値を挙げて回答してください
- 「〜を確認してください」「最新のデータを〜」のような回答は**絶対に禁止**です
- データが与えられているのだから、そのデータを使って具体的に答えてください

【データの読み方】
- L4F: 後半4ハロンのラップ評価（高いほど優秀なラップを刻んでいる）
- ポテンシャル: 能力値（高いほど能力が高い）
- 巻き返し: 前走で不利があった度合い（高いほど今回巻き返しが期待できる）
- レースレベルA/B: ハイレベル戦、C/D: 標準〜低レベル

【回答の仕方】
- 質問に対して、データから該当する馬を特定して回答
- 「〇番の△△は前走でL4F=XX.Xと優秀で〜」のように具体的に
- 複数該当する場合はランキング形式で
` : `
- 質問に簡潔に回答してください

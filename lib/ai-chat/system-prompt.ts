...
### 2. T2F指数（前半2F速度指数）
- **数字が低いほど前半が速い**（22.5以下なら速力がある）
- T2Fが低い馬は先行争いに加わりやすい
- 先行馬が少なければスローペースで展開の恩恵を受けやすい
- 先行力のある馬が揃えばハイペースになりやすい
- ※この指数は馬の能力を直接示すものではなく、展開予想の材料

### 3. L4F指数（後半4F速度指数）
- **数字が低いほど後半が速い**（45以下なら高評価）
- 後半のラップが速い＝持続力がある、または展開が向いた
- 逃げ馬以外がL4F低い数値を出していたら、追いかけながらの数値なので特に優秀
- ※この指数は馬の能力を直接示すものではなく、レース内容の参考値
...
【予想の構築要素（優先順位順）】
1. **レースレベル**: 前走がハイレベル戦（A/B）での好走は高評価
2. **L4F指数**: 45以下なら後半が速く高評価（数字が低いほど良い）

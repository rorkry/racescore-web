                                        ...
                                        <Tab.Panel key={raceNo}>
                                          <div className="overflow-auto bg-white rounded-xl shadow-md">
                                            <table className="w-full text-left border-collapse border border-black text-xs">
                                              <thead>
                                                <tr>
                                                  <th className="px-1 py-1 border border-black bg-gray-100 text-black">馬番</th>
                                                  <th className="px-1 py-0.5 border border-black bg-gray-100 text-black text-xs">
                                                    馬名
                                                  </th>
                                                  <th className="px-1 py-0.5 border border-black bg-gray-100 text-black text-xs">騎手</th>
                                                  {['前走','2走前','3走前','4走前','5走前'].map((label, i) => (
                                                    <th
                                                      key={i}
                                                      className="px-1 py-0.5 border border-black bg-gray-100 text-black text-xs"
                                                    >
                                                      {label}
                                                    </th>
                                                  ))}
                                              ...
                                              <tbody>
                                                {horses.map((horse, idx) => (
                                                  <tr key={idx} className="odd:bg-white even:bg-gray-50">
                                                    {/* 馬番セル */}
                                                    <td
                                                      className={`w-8 px-0 py-0 border border-black text-center align-middle ${frameColor[horse.entry['枠番'] ?? ''] ?? 'bg-gray-300 text-black'}`}
                                                    >
                                                      {horse.entry['馬番']}
                                                    </td>
                                                    {/* 馬名・詳細 */}
                                                    <td className="relative px-1 py-0.5 border text-black align-top whitespace-nowrap text-xs">
                                                      <div className="font-bold">
                                                        <span className={
                                                          horse.past.some(rr => favorites.has(rr['レースID(新/馬番無)']?.trim() || ''))
                                                            ? 'text-red-600'
                                                            : ''
                                                        }>
                                                          {horse.entry['馬名']}
                                                      ...
                                                      <div className="text-sm">
                                                        {horse.entry['性別']}{horse.entry['馬齢']}<br/>
                                                        {horse.entry['調教師']}／{horse.entry['所属']}
                                                      </div>
                                                      {labels[idx] === 'くるでしょ' ? (
                                                        <button className="absolute bottom-1 left-0 right-0 inline-flex justify-center items-center text-sm font-semibold px-3 py-1 rounded-lg shadow-md bg-yellow-500 text-white hover:bg-yellow-600 transition">
                                                          {labels[idx]}
                                                        </button>
                                                      ) : labels[idx] === 'めっちゃきそう' ? (
                                                        <button className="absolute bottom-1 left-0 right-0 inline-flex justify-center items-center text-sm font-semibold px-3 py-1 rounded-lg shadow-md bg-red-500 text-white hover:bg-red-600 transition">
                                                          {labels[idx]}
                                                        </button>
                                                      ) : (
                                                        <div className={`absolute bottom-1 left-0 right-0 text-center text-sm font-semibold inline-block px-2 py-0.5 rounded-full ${
                                                          labels[idx] === 'ちょっときそう'
                                                            ? 'text-orange-500 bg-orange-100'
                                                            : labels[idx] === 'こなそう'
                                                            ? 'text-blue-800 bg-blue-100'
                                                            : labels[idx] === REMAIN_LABEL
                                                    ...
                                                    {/* 騎手・斤量 */}
                                                    <td className="px-1 py-0.5 border border-black text-xs whitespace-nowrap text-black font-medium">
                                                      {horse.entry['騎手']}
                                                      {horse.entry['斤量'] && (
                                                        <span className="ml-1 text-xs text-gray-600">{horse.entry['斤量']}kg</span>
                                                      )}
                                                    </td>
                                                    {/* 過去５走 */}
                                                    {horse.past.map((r, j) => {
                                                      // 距離差分計算
                                                      const currDistEntry = horse.entry['距離']?.trim() || '';
                                                      ...
                                                      // 馬場(芝/ダ等)違い判定
                                                      const currSurface = horse.entry['馬場']?.trim() || '';
                                                      ...
                                                      const levelRaw = r['レース印３']?.trim() || ''
                                                      const starCount = levelToStars(levelRaw)
                                                      const stars = starCount > 0 ? '★'.repeat(starCount) : '-'
                                                      let starColorClass = 'text-black'
                                                      switch (starCount) {
                                                        case 5: starColorClass = 'text-red-500'; break
                                                        case 4: starColorClass = 'text-orange-500'; break
                                                        case 3: starColorClass = 'text-blue-800'; break
                                                        case 2: starColorClass = 'text-gray-700'; break
                                                        case 1: starColorClass = 'text-gray-400'; break
                                                      }
                                                      ...
                                                      // 強い競馬判定
                                                      const rpc = parseFloat(r['RPCI'] || '0');
                                                      const pci3 = parseFloat(r['PCI3'] || '0')
                                                      const deltaPCI = pci3 - rpc
                                                      const passNums = [r['2角'], r['3角'], r['4角']]
                                                        .map(x => parseInt((x || '').replace(/[^\d]/g, ''), 10))
                                                        .filter(n => !isNaN(n))
                                                      ...
                                                      const _isStrong = deltaPCI >= 3 && avgPass <= 4
                                                      const passing = [
                                                        r['2角']?.trim(),
                                                        r['3角']?.trim(),
                                                        r['4角']?.trim()
                                                      ...
                                                      const finishColorClass = 'text-black';
                                                      return (
                                                      ...
                                                      <div className="flex items-center mb-1">
                                                        <div className={`text-sm font-medium ${j === 0 && (isDistDiff || isSurfaceDiff) ? 'text-green-500' : ''}`}>
                                                          ...
                                                          {finishPos && (
                                                            <div className={`${finishColorClass} text-lg font-semibold absolute bottom-1 right-1`}>
                                                              {finishPos}
                                                            </div>
                                                          )}
                                                          ...
                                                          <div className={`${starColorClass} mb-1`}>{stars}</div>
                                                          ...
                                                          {(() => {
                                                            ...
                                                            return (
                                                              <div className={`text-xs mb-1 ${passColorClass}`}>
                                                                {passing}
                                                              </div>
                                                            )
                                                          })()}
                                                    ...
                                                    {horse.past.length < 5 &&
                                                      Array.from({ length: 5 - horse.past.length }).map((_, k) => (
                                                        <td
                                                          key={`empty-${k}`}
                                                          className="align-top px-1 py-0.5 border border-black bg-white text-black text-xs"
                                                        >
                                                          &nbsp;
                                                        </td>
                                                    ))}
                                            ...
                                            <div className="overflow-auto bg-white rounded-xl shadow-md">
                                              ...
                                              <thead>
                                                <tr>
                                                  ...
                                                  {/* 印ヘッダー */}
                                                  <th className="px-1 py-1 border border-black bg-gray-100 text-black w-[3rem]">印</th>
                                                  <th className="px-2 py-1 border border-black bg-gray-100 text-black">騎手</th>
                                                  {['前走','2走前','3走前','4走前','5走前'].map((label, i) => (
                                                ...
                                                <tbody>
                                                  {horses.map((horse, idx) => (
                                                    <tr key={idx} className="odd:bg-white even:bg-gray-50">
                                                      {/* 馬番セル */}
                                                      <td
                                                        className={`w-8 px-0 py-0 border border-black text-center align-middle font-semibold ${
                                                          frameBgStyle[horse.entry['枠番'] ?? ''] ?? 'bg-gray-300 text-black'
                                                        }`}
                                                      >
                                                        {horse.entry['馬番']}
                                                      </td>
                                                      {/* 馬名・詳細 */}
                                                      <td className="relative px-2 py-1 border text-black align-top whitespace-nowrap">
                                                        <div className="font-bold">
                                                          <span className={
                                                            horse.past.some(rr => favorites.has(rr['レースID(新/馬番無)']?.trim() || ''))
                                                              ? 'text-red-600'
                                                      ...
                                                      {/* 印セル */}
                                                      <td className="px-1 py-0.5 border border-black text-center w-10">
                                                        {(() => {
                                                          ...
                                                          return (
                                                            <select
                                                              ...
                                                              /* 文字が隠れないように矢印を消し、右側に余白を確保 */
                                                              className="appearance-none bg-white border border-gray-300 rounded w-[2rem] h-[2rem] text-center text-m font-bold text-black leading-tight"
                                                            >

...
export async function POST(request: NextRequest) {
  try {
    ...
    // 上限チェック（無料ユーザー）
    const count = await db.prepare('SELECT COUNT(*) as cnt FROM favorite_horses WHERE user_id = ?').get<{ cnt: number }>(user.id);
    if (count && count.cnt >= FREE_LIMIT) {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    await db.prepare('UPDATE favorite_horses SET note = ? WHERE user_id = ? AND horse_name = ?').run(note || null, user.id, horseName);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await db.prepare('DELETE FROM favorite_horses WHERE user_id = ? AND horse_name = ?').run(user.id, horseName);
    return NextResponse.json({ success: true });
  } catch (error) {
